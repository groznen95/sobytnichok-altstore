...
notes = (
    "Виджет работает и без общего контейнера приложения. Под бесплатной "
    "учётной записью Apple разрешение App Group не выдаётся, и виджет был "
    "пустым; теперь он сам берёт ближайшее событие из открытого каталога и "
    "честно помечает его словом «каталог». Как только контейнер появится, "
    "вернутся личные подписки.\n\n"
    "Даты считаются по границам суток, а не по прошедшим часам: виджет "
    "писал «сегодня» там, где приложение писало «завтра»."
)
